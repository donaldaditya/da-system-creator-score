...
export function buildExportWorkbook(scoredCreators: import("@/types/creator").ScoredCreator[]): XLSX.WorkBook {
  ...
  const breakdownData = scoredCreators.map((c) => ({
    Handle: c.handle,
    "Composite Score": c.composite.total,
    "Score Badge": c.composite.badge,
    "Branding Total": c.branding.total,
    "Engagement Score": c.branding.signals.engagement?.normalizedScore ?? "",
    "Posting Score": c.branding.signals.posting?.normalizedScore ?? "",
    "Reach Score": c.branding.signals.reach?.normalizedScore ?? "",
    "Commerce Total": c.conversion.total,
    "GMV Score": c.conversion.signals.gmv?.normalizedScore ?? "",
    "CTR Score": c.conversion.signals.ctr?.normalizedScore ?? "",
    "CTOR Score": c.conversion.signals.ctor?.normalizedScore ?? "",
    "Branding Completeness %": Math.round(c.branding.dataCompleteness * 100),
    "Commerce Completeness %": Math.round(c.conversion.dataCompleteness * 100),
  }));
